...
import { deleteEmployee } from "../../../app/store/Employees/EmployeesThunk";
...
const VisibleModalBox = ({setModalDelete, id}: props) => {
    ...
    const handleSubmit = async () => {
        await dispatch(deleteEmployee(id._id));
    ...
    return (
        <ModalInfo>
                <div>
                    <h1>Delete Employee profile</h1>
                    <IoCloseOutline onClick={() => setModalDelete(false)} />
                </div>
                <p>Employee profile will be deleted and cannot be recovered! Are you sure you want to continue?</p>
                <div>
...
export function DeleteEmployeeModal({modalDelete, setModalDelete, id}: props) {
    return (
        <>
